...
1. Faster type testing. It avoids a memory access, but I'm not sure /how/
   ...
   - nil check (but this can be made non-memory-accessing even without k-bit
     tags by making nil a statically-allocated value)

   - builtins, potentially. we could easily have builtins be immediate values.

   Actually, these 2 exceptions suggests a 2-bit encoding:
   - one tag for fixnum
   - one tag for builtins
   - one tag for nil (?)
   - one tag for everything else
